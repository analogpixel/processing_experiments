...
void rdraw(float x_offset, float y_offset, float w, float h, float shift, int c) {
  ...
  rdraw(x,y, w/2, h/2, shift,  c-1);
  rdraw(x ,y, w/2, h/2, shift+1, c-1);
  rdraw(x,y, w/2, h/2, shift+2, c-1);
...
void draw() {
  background(0);
  rdraw(0,0, width, height, shift, 6);
  shift+=.01;
}
